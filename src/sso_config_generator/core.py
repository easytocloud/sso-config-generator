...
class SSOConfigGenerator:
    ...
    def _store_generator_config(self, base_path: Path) -> None:
        """Write .sso-config-generator.ini in base_path so that re-running from
        that directory automatically reuses the same settings.

        The file is written with skip_sso_name = true and no unified_root because
        base_path IS the root — the caller is already positioned there.

        Any legacy .generate-sso-config YAML file in the same directory is removed.
        """
        ini = configparser.ConfigParser()
        ini['sso-config-generator'] = {}
        section = ini['sso-config-generator']

        section['create_directories'] = 'true'
        section['use_ou_structure'] = 'true' if self.use_ou_structure else 'false'
        if self.developer_role_name:
            section['developer_role_name'] = self.developer_role_name
        section['create_repos_md'] = 'true' if self.create_repos_md else 'false'
        # When running from base_path the SSO name dir is the CWD itself
        section['skip_sso_name'] = 'true'
        if self.sso_name:
            section['sso_name'] = self.sso_name
        section['region'] = self.region
        if self.profile_name != 'sso-browser':
            section['profile'] = self.profile_name
        if self._explicit_sso_session_name:
            section['sso_session_name'] = self._explicit_sso_session_name

        with open(base_path / '.sso-config-generator.ini', 'w') as f:
            f.write("# Generated by sso-config-generator — re-run from this directory to refresh.\n")
            f.write("# Edit to change behaviour; CLI flags always take precedence.\n\n")
            ini.write(f)

        # Remove legacy file if present
        legacy = base_path / '.generate-sso-config'
        if legacy.exists():
            legacy.unlink()
            print(f"  Migrated .generate-sso-config → .sso-config-generator.ini")
